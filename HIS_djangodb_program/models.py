...
class patient_information(models.Model):     #患者信息
    pid = models.CharField(primary_key=True, max_length=6)   #自增？
    IDnumber = models.CharField(max_length=18)     #定长字段待我研究研究
    pname = models.CharField(max_length=10)      #变长字段同上
    psex = models.BooleanField()
    page = models.PositiveIntegerField()
    pphone = models.CharField(max_length = 11)
    ppast = models.TextField(blank=True)
    pallergy = models.TextField(blank=True)
    ppassword = models.CharField(max_length = 20, blank=True)

class departments(models.Model):     #科室表
    DEPARTMENT_CHOICES = (
        (1, '内科'),
        (2, '外科'), 
        (3, '儿科'),
    ...
    deptid = models.IntegerField(primary_key=True, choices=DEPARTMENT_CHOICES)
    deptname = models.CharField(max_length=10)

class notice(models.Model):  #通知表
    deptid = models.ForeignKey(departments, on_delete=models.CASCADE, related_name = 'not_deptid')
    ntime = models.DateTimeField()
    ncontent = models.TextField(blank = True)
    class Meta:
        db_table = 'notice'
        unique_together = ['deptid', 'ntime']
